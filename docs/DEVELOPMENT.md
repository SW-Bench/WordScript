...
- reale Fehltranskripte aus problematischen Profilen werden ueber den Korpus in `src-tauri/tests/fixtures/regression_transcripts.json` plus Loader in `core::regression_corpus` abgefahren; neue Bias-Filter gehoeren in den Korpus und in passende synthetische Tests in `core::transcription_hints::tests` und `core::text_rules::tests`, nicht nur in `core::transform::tests`; wer ein neues Bias-Filter- oder Guardrail-Verhalten einbaut, muss den Korpus um mindestens einen realen oder realitaetsnahen Fall erweitern und die Korpus-Assertions entsprechend anpassen
- Profil-Bias-Policy-Aenderungen muessen `analyze_transcription_bias_with_mode` (Preview-Build), `transcription_prompt_for_request` (Runtime-Build), `analyze_document_with_context` (UI-Pfad) und die `BiasMode`-Migration in `AppConfig::migrate_bias_policy` gemeinsam treffen; eine Aenderung nur in einem der drei Pfade ist ein Vertragsbruch zwischen Produkt-UI, Profil-Health und Runtime
- wer `profile_health_acknowledged_flags` anfasst, muss `acknowledge_profile_health_flag` / `unacknowledge_profile_health_flag` (Persistenz), `get_profile_health` (Merge mit Request) und die `ProfileHealthLevel`-Ableitung (Red/Green-Logik) gemeinsam aktualisieren
...
- Der Zielmonitor fuer gemerkte Overlay-Positionen muss aus dieser gespeicherten logischen Drag-Referenz gegen die verfuegbaren Monitor-Work-Areas aufgeloest werden. `current_monitor()` allein ist auf Linux-/Wayland-Multi-Monitor-Pfaden nicht robust genug. Bei Monitor-Identity-Miss (Reconnect/Sleep/Treiber-Reenumerateation aendert Name/Fingerprint) muss der Restore nicht auf Primary fallen, sondern den Monitor per Koordinaten-Containment aus der gespeicherten logischen Drag-Referenz rederivieren (`resolve_overlay_monitor` → `resolve_overlay_monitor_id`); nur Preset-Mode ohne Identity-Match faellt auf Primary.
